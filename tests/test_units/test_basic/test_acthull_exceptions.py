...
class TestActHullInputValidation:
    ...
    def test_missing_bounds_raises_error(self, relu_hull_class):
    ...
    def test_missing_both_bounds_raises_error(self, leakyrelu_hull_class):
    ...
    def test_missing_input_constraints_with_bounds(self, relu_hull_class):
    ...
    def test_too_few_constraints_error(self, relu_hull_class):
    ...
    def test_neither_constraints_nor_bounds_raises_error(self, relu_hull_class):
...
class TestActHullBoundValidation:
    ...
    def test_non_1d_lower_bounds_raises_error(self, relu_hull_class):
    ...
    def test_non_1d_upper_bounds_raises_error(self, elu_hull_class):
    ...
    def test_bounds_size_mismatch_raises_error(self, relu_hull_class):
    ...
    def test_bounds_reversed_raises_error(self, leakyrelu_hull_class):
...
class TestActHullConstraintValidation:
    ...
    def test_too_few_constraints_raises_error(self, relu_hull_class):
    ...
    def test_unbounded_polytope_raises_error(self, relu_hull_class):
...
class TestActHullDoubleOrders:
    ...
    def test_double_orders_requires_multi_neuron(self, leakyrelu_hull_class):
    ...
    def test_single_neuron_multi_neuron_valid_combination(self, relu_hull_class):
        """Test valid single and multi-neuron mode combinations."""
        lb = np.array([-1.0, -1.0])
        ub = np.array([1.0, 1.0])

        # Single neuron only is valid
        hull_sn = relu_hull_class(
            if_cal_single_neuron_constrs=True, if_cal_multi_neuron_constrs=False
        )
...
class TestActHullReversedOrderCaching:
    ...
    def test_relu_consistent_across_calls(self, relu_hull_class):
    ...
    def test_leakyrelu_consistent_across_dimensions(self, leakyrelu_hull_class):
...
class TestActHullExceptionRecovery:
    ...
    def test_single_neuron_valid_bounds_tolerance(self, relu_hull_class):
    ...
    def test_multi_neuron_degenerate_recovery(self, relu_hull_class):
        """Test recovery from degenerate polytope with fractional arithmetic."""
        hull = relu_hull_class(if_cal_multi_neuron_constrs=True, if_cal_single_neuron_constrs=False)
        ...
        # Should either succeed with valid constraints or raise a known error
        assert raised or isinstance(constraints, np.ndarray)

    def test_maxpool_multi_neuron_soundness_with_recovery(self, maxpool_hull_class):
...
class TestActHullConstraintInterpolation:
    ...
    def test_relu_single_neuron_constraint_properties(self, relu_hull_class):
    ...
    def test_sigmoid_single_neuron_finite_outputs(self, sigmoid_hull_class):
...
class TestActHullEdgeDimensionHandling:
    ...
    def test_single_dimension_input(self, relu_hull_class):
    ...
    def test_high_dimension_input(self, elu_hull_class):
...
class TestActHullExceptionPaths:
    ...
    def test_none_result_error_handling(self, relu_hull_class):
    ...
    def test_maxpool_complex_multi_neuron_recovery(self, maxpool_hull_class):
        ...
        for lb, ub in test_cases:
            try:
                ...
                pass  # Some bound configs may legitimately fail

        # At least one test case should succeed
        assert succeeded_count > 0, "All maxpool multi-neuron recovery cases failed"


class TestActHullConsistency:
    """Test consistency of ActHull computations across modes."""

    def test_single_vs_multi_neuron_consistent_column_format(self, relu_hull_class):
    ...
    def test_combined_mode_superset_property(self, leakyrelu_hull_class):
        """Test that combined mode produces superset of constraints."""
        lb = np.array([-1.0, -1.0])
        ub = np.array([1.0, 1.0])

        hull_both = leakyrelu_hull_class(
        ...
        hull_single = leakyrelu_hull_class(
            if_cal_single_neuron_constrs=True, if_cal_multi_neuron_constrs=False
        )
